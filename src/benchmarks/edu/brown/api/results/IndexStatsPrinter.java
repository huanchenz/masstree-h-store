...
/**
 * Poll the system at every interval to collect index information 
 *
 */
public class IndexStatsPrinter implements BenchmarkInterest {
    private static final Logger LOG = Logger.getLogger(IndexStatsPrinter.class);
	...
	new ColumnInfo("INDEX_TYPE", VoltType.STRING),       
	new ColumnInfo("IS_UNIQUE", VoltType.INTEGER),
	new ColumnInfo("ENTRY_COUNT", VoltType.INTEGER),
	new ColumnInfo("MEMORY_ESTIMATE", VoltType.INTEGER),
    };
    ...
    public IndexStatsPrinter(Client client, File outputPath) {
        this.client = client;
    ...
    public String formatFinalResults(BenchmarkResults br) {
        ...
        LOG.info("Wrote CSV index stats periodically to '" + this.outputPath.getAbsolutePath() + "'");
        return (null);
    }
        ...
        ProcedureCallback callback = new ProcedureCallback() {
            ...
            public void clientCallback(ClientResponse clientResponse) {
                if (clientResponse.getStatus() != Status.OK) {
                    LOG.warn("Failed to get index stats", clientResponse.getException());
                    return;
                }
                if (debug.val)
                    LOG.debug("Updating index stats information [interval=" + interval + "]");
                ...
                while (vt.advanceRow()) {
                    ...
                    IndexStatsPrinter.this.results.add(row);
                };
