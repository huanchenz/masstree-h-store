...
def getDependencies(filename, cppflags, sysprefixes):
    command = "g++ %s -std=gnu++0x -MM %s" % (cppflags, filename)
...
def buildMakefile(CTX):
    ...
    for filename in input_paths:
        ...
        if (jni_objname == "objects/indexes/tableindexfactory.co"):
            makefile.write("\t$(CCACHE) $(COMPILE.cpp) -std=gnu++0x %s -o $@ %s\n" % (CTX.EXTRAFLAGS, filename))
        else:
            makefile.write("\t$(CCACHE) $(COMPILE.cpp) %s -o $@ %s\n" % (CTX.EXTRAFLAGS, filename))
        makefile.write(static_objname + ": " + filename + " " + " ".join(mydeps) + "\n")
        if (static_objname == "static_objects/indexes/tableindexfactory.co"):
            makefile.write("\t$(CCACHE) $(COMPILE.cpp) -std=gnu++0x %s -o $@ %s\n" % (CTX.EXTRAFLAGS, filename))
        else:
            makefile.write("\t$(CCACHE) $(COMPILE.cpp) %s -o $@ %s\n" % (CTX.EXTRAFLAGS, filename))
        makefile.write("\n")
